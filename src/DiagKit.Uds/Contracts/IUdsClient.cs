...
/// <summary>
/// 同步 UDS 诊断客户端接口，可发出诊断请求并等待响应。
/// <br/>Synchronous UDS diagnostic client interface that can issue a diagnostic request and wait for a response.
/// </summary>
/// <remarks>
/// 实现类负责通过底层传输器（如 ISO 15765 DoCAN 或 ISO 13400 DoIP）发送 UDS 请求消息，
/// 并等待对应的响应。该接口提供了 RequestPending（0x78）和 BusyRepeatRequest（0x21）
/// 状态机的自动处理。同一时间只允许一个请求在进行中，并发调用将抛出异常。
/// <br/>Implementing classes are responsible for sending UDS request messages through an
/// underlying transporter (e.g. ISO 15765 DoCAN or ISO 13400 DoIP) and waiting for the
/// corresponding response. Automatic handling of RequestPending (0x78) and
/// BusyRepeatRequest (0x21) state machines is provided. Only one request is allowed
...
public interface IUdsClient
{
    ...
    /// <summary>
    /// 发送一个 UDS 请求并返回原始响应字节。
    /// 当响应被抑制时返回空 <see cref="ReadOnlyMemory{T}"/>。
    /// <br/>Send a UDS request and return the raw response bytes.
    ...
    /// </param>
    /// <returns>
    /// 来自 ECU 的原始响应字节。如果响应被抑制，则返回空 <see cref="ReadOnlyMemory{T}"/>。
    /// <br/>The raw response bytes from the ECU. Empty <see cref="ReadOnlyMemory{T}"/> if
    ...
    /// </exception>
    ReadOnlyMemory<byte> SendRequest(ReadOnlyMemory<byte> request, bool? suppressResponse = null, CancellationToken cancellationToken = default);
}
